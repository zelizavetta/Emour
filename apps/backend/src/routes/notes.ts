...
router.post('/', asyncHandler(async(req: Request, res: Response) => {
    ...
    if (result.rowCount === 0) {
        throw ApiError.internal("Internal server error", { details: "Create note record failed" })
    }
...
router.patch('/title/:noteId', asyncHandler(async(req: Request, res: Response) => {
    ...
    if (result.rowCount === 0) {
        throw ApiError.internal("Internal server error", { details: "Update title note record failed" })
    }
...
router.patch('/text/:noteId', asyncHandler(async(req: Request, res: Response) => {
    ...
    if (result.rowCount === 0) {
        throw ApiError.internal("Internal server error", { details: "Update text note record failed" })
    }
...
router.get('/all', asyncHandler(async(req: Request, res: Response) => {
    ...
    `)
    if (result.rowCount === 0) {
        throw ApiError.notFound("Not found", { details: "No notes in db" })
    }
...
router.get('/:noteId', asyncHandler(async(req: Request, res: Response) => {
    ...
    `, [noteId])
    if (result.rowCount === 0) {
        throw ApiError.notFound("Not found", { details: "No note in db" })
    }
